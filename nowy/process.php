...
if($page=='view'){
    ...
    $id=$_SESSION['id'];
    $result = $mysqli->query("SELECT * FROM list_of_order where (status!='2' AND status!='3') AND (user_id=$id or user_id=$houses_id*-1)");
    ...
    while($row = $result->fetch_assoc()){
        ...
        <tr>
            ...
            <td><input type="number"/><?php echo $row['price'] ?></td>
            
            <td><input type="number"/><?php echo $row['effort_hours'] ?></td>
