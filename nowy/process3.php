...
if($page=='view'){
    ...
    $id=$_SESSION['id'];
    $result = $mysqli->query("SELECT * FROM list_of_order where (status='2' OR status='3') AND house_id=$houses_id");
...
do{
    ...
    $houses_id=$_SESSION['houses_id'];
    $id=$_SESSION['id'];
    $result = $mysqli->query("SELECT * FROM list_of_order where status='2' AND house_id=$houses_id");
    $usersql="SELECT name FROM users where houses_id=$houses_id";
    $usertable=$mysqli->query($usersql);
